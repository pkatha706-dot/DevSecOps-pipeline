...
| 8 | [Migrated providers, Anthropic to Gemini](#inc-08-migrated-providers-anthropic-to-gemini) | Vendor migration | [`38dc125`](https://github.com/pkatha706-dot/DevSecOps-pipeline/commit/38dc125) |
...
**Investigation:** Diffed `HEAD` against `origin/main` before touching anything further. The ZAP step had been rewritten directly on GitHub, replacing the `zaproxy/action-baseline` marketplace action with a raw `docker run` against `ghcr.io/zaproxy/zaproxy:stable`, and renaming the output artifact to `zap-scan-results` / `zap-report.json`.
...
## INC-08: Migrated providers, Anthropic to Gemini
...
**Verification:** N/A at the code level; confirmed by the next run reaching a different (unrelated) failure, meaning this specific error stopped recurring.
...
**Symptom:** The GitHub Issue the pipeline opened contained only an Executive Summary, no findings tables and no remediation list, despite the prompt explicitly requesting five sections.
...
**Fix:** Removed `thinking_config` entirely rather than guess at a model-specific valid range, and raised `max_output_tokens` to 8192 instead, giving enough headroom for both an automatic reasoning pass and the full visible report without depending on an undocumented constraint.
